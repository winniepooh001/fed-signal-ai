...
class UniversalLLMUsageTracker(BaseCallbackHandler):
    ...
    def on_llm_start(self, serialized: Dict[str, Any], prompts: List[str], **kwargs) -> None:
        ...
        logger.debug(f"LLM call started: {model_info['provider']}/{model_info['model']}")
    ...
    def on_llm_end(self, response, **kwargs) -> None:
        ...
        try:
            ...
            cost_estimate = self._calculate_cost_universal(provider, model_name, prompt_tokens, completion_tokens)
            ...
            if self.db_manager:
                self.db_manager.save_llm_usage(
                    ...
                    response_data={'provider': provider, 'generations': self._count_generations(response)},
                    cost_estimate=cost_estimate
                )

            logger.info(f"LLM call completed: {provider}/{model_name} - {total_tokens} tokens, ${cost_estimate:.4f}")
    ...
    def _extract_model_info(self, serialized: Dict[str, Any], kwargs: Dict[str, Any]) -> Dict[str, str]:
        ...
        model_sources = [
            ...
            getattr(kwargs.get('run_manager'), 'name', None) if kwargs.get('run_manager') else None
        ]
    ...
    def _detect_provider(self, model_name: str, serialized: Dict[str, Any]) -> str:
        ...
        # Default fallback
        logger.warning(f"Could not detect provider for model '{model_name}', defaulting to 'unknown'")
        return 'unknown'
        ...
        for strategy in strategies:
            try:
                ...
                if result and result.get('total_tokens', 0) > 0:
                    logger.debug(f"Token extraction successful using {strategy.__name__}")
                    return result
            except Exception as e:
    ...
    def _strategy_response_metadata(self, response, provider: str) -> Optional[Dict[str, int]]:
    ...
    def _strategy_usage_metadata(self, response, provider: str) -> Optional[Dict[str, int]]:
    ...
    def _strategy_generations(self, response, provider: str) -> Optional[Dict[str, int]]:
    ...
    def _strategy_provider_specific(self, response, provider: str) -> Optional[Dict[str, int]]:
    ...
    def _strategy_estimate_fallback(self, response, provider: str) -> Optional[Dict[str, int]]:
    ...
    def _normalize_token_fields(self, usage: Dict[str, Any]) -> Dict[str, int]:
        ...
        completion_fields = ['completion_tokens', 'output_tokens', 'completion_token_count', 'generated_tokens']
    ...
    def _calculate_cost_universal(self, provider: str, model: str, prompt_tokens: int, completion_tokens: int) -> float:
        ...
        if not model_pricing:
            if 'default' in provider_pricing:
                ...
                logger.warning(f"No pricing found for {provider}/{model}, using default rates")
