...
class FilterValidator:
    ...
    def validate_and_fix_filters(filters: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for column, column_filters in column_groups.items():
            if len(column_filters) == 1:
                ...
                logger.info(
                    f"Fixing {len(column_filters)} filters for column '{column}'"
                )
                fixed_filter = FilterValidator._fix_column_filters(
    ...
    @staticmethod
    def _fix_column_filters(
        column: str, filters: List[Dict[str, Any]]
        ...
        if all(ftype == "equals" for ftype in filter_types):
            ...
            logger.info(
                f"Converting {len(filters)} equals filters to 'in' filter for {column}: {values}"
            )
        ...
        # Case 2: Greater than + Less than → Convert to range filter
            ...
            logger.info(
                f"Converting greater_than + less_than to range filter for {column}: {min_val}-{max_val}"
            )
            return {
                "type": "range",
                "column": column,
        ...
        # Case 3: Mix of 'in' with other types → Keep 'in', remove others
            ...
            logger.warning(
                f"Found 'in' filter mixed with others for {column} - keeping only 'in' filter"
            )
            return in_filter
        ...
        # Case 4: Multiple range filters → Use the most restrictive one
            ...
            for f in filters[1:]:
                if f.get("min_value", 0) > best_filter.get("min_value", 0) or f.get(
                    "max_value", float("inf")
                ) < best_filter.get("max_value", float("inf")):
                    best_filter = f

            logger.info(
                f"Multiple range filters for {column} - using most restrictive: "
        ...
        # Case 5: Complex mix → Try to be smart about it
            ...
            logger.warning(
                f"Complex filter mix for {column}: {filter_types} - applying heuristics"
            )
            ...
            for preferred_type in priority_order:
                matching_filters = [
                    f for f in filters if f.get("type") == preferred_type
                ]
                if matching_filters:
